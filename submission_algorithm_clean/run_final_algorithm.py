...
def build_final_model() -> PairwiseCorrectedUtility:
    return PairwiseCorrectedUtility(
        base_model=RandomForestUtility(
            n_trees=15,
            max_depth=5,
            min_samples_leaf=10,
        ...
        pairwise_model=PairwiseLogisticRanker(
            epochs=80,
            learning_rate=0.08,
            l2=0.003,
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train final counter-UAV strategy recommendation model and predict test strategies.")
    parser.add_argument("--data-dir", default="Data", help="Directory containing the training and test xlsx files.")
    parser.add_argument("--output-dir", default="outputs/final_submission", help="Directory to write recommendation CSV and summary JSON.")
    ...
    bundle = load_competition_data(data_dir)
    utility = interception_priority_utility(bundle.train_metric1, bundle.train_metric2, secondary_weight=0.05)
    ...
    rec_path = output_dir / "test_recommendations.csv"
    with rec_path.open("w", newline="", encoding="utf-8-sig") as f:
        ...
        for i, (strategy, score_row) in enumerate(zip(pred, scores), start=1):
            writer.writerow([i, int(strategy), *[round(float(v), 8) for v in score_row]])

    summary = {
        "method": "interception_priority_core25_pairwise_corrected",
        ...
        "raw_feature_count": int(bundle.train_x.shape[1]),
        "selected_feature_count": int(train_x.shape[1]),
        "engineered_feature_count": int(train_x.shape[1] - bundle.train_x.shape[1]),
        "secondary_weight_metric2": 0.05,
        "pairwise_weight": 0.35,
        "strategy3_penalty": 0.05,
        "test_prediction_counts": {str(int(k)): int(v) for k, v in zip(*np.unique(pred, return_counts=True))},
        "selected_features": selected_names,
        "output_file": str(rec_path),
    }
